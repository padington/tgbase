...
- Resumable sets: `resumableScrState` = all `scr_*` except consent/intro/delete-confirm; `resumableMoodState` (PHQ-9-owned positions, recorded in `Mood.ResumeState`) = `mood_question` | `mood_crisis` | `mood_q10`; `resumableEatState` = the three `eat_*` question series. WHO-5/GAD-7 and all three eating runs derive their position from the answer count — no recorded position needed.
